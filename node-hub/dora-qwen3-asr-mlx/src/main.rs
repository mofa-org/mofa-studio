//! Dora ASR Node using MLX-based Qwen3-ASR
//!
//! This node provides speech-to-text transcription optimized for Apple Silicon
//! using the Qwen3-ASR model via MLX (Metal acceleration). Supports 30+ languages.
...
use config::Config;
use engines::Qwen3AsrEngine;

fn main() -> Result<()> {
    // Initialize logging
    env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("info")).init();

    log::info!("Starting dora-qwen3-asr-mlx node...");
    ...
    let mut engine = if config.warmup {
        log::info!("Pre-initializing Qwen3-ASR engine...");
        let start = Instant::now();
        match Qwen3AsrEngine::new(&config.model_dir, &config.language) {
            Ok(e) => {
            ...
            Event::Input { id, data, metadata } => {
                ...
                match input_id {
                    "audio" => {
                        ...
                        if engine.is_none() {
                            log::info!("Lazily initializing Qwen3-ASR engine...");
                            let start = Instant::now();
                            match Qwen3AsrEngine::new(&config.model_dir, &config.language) {
                                Ok(e) => {
                                    log::info!(
                        ...
                        // Check for per-request language override
                        let language_override = metadata
                            .parameters
                            .get("language")
                            .and_then(|p| match p {
    ...
    log::info!("dora-qwen3-asr-mlx node stopped");
    Ok(())
}

fn process_audio(
    node: &mut DoraNode,
    engine: &mut Qwen3AsrEngine,
    samples: &[f32],
    ...
    let language = language_override
        .map(|s| s.to_string())
        .unwrap_or_else(|| engine.language().to_string());

    log::info!(
        "Processing audio: {:.2}s, language={}, question_id={:?}, segment={:?}",
        audio_duration,
        language,
        question_id,
        segment
    ...
    let start = Instant::now();
    let transcription = engine.transcribe_with_language(samples, &language)?;
...
fn handle_control_command(
    ...
    match command.as_str() {
        ...
        "cleanup" | "reset" => {
            send_log(node, "INFO", "Engine reset (no-op for Qwen3-ASR)")?;
        }
        _ => {
